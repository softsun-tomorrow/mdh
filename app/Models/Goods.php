...
//
    ...
    {
        return $this->hasMany('App\\Models\\GoodsSpu');
    }
    ...
    public function getPromInfoAttribute()
    {
        if($this->prom_type && $this->prom_id){
            ...
            if ($goodsPromFactory->checkPromType($this->prom_type)) {
                //$goodsPromLogic = $goodsPromFactory->makeModule($this); // 活动状态status != 1 时有bug存在
                //$goodsPromModel = $goodsPromLogic->getPromModel();

                if(true){ //$goodsPromModel
                    //活动类型：0默认，1=限时抢购，2=拼团，3=抽奖
                    if ($this->prom_type == 1) {
                        $prom_tag = '特惠';
                        $goodsPromModel = FlashSale::find($this->prom_id);
                    } elseif($this->prom_type == 2) {
                        $goodsPromModel = TeamActivity::find($this->prom_id);
                        $prom_tag = $goodsPromModel->needer .'人团';
     ...
     * 不能用，添加商品时产生一个bug
       public function cat1()
        {
            return $this->belongsTo('App\Models\Category','cat1','id');
        }

        public function cat2()
        {
            return $this->belongsTo('App\Models\Category','cat2','id');
        }
